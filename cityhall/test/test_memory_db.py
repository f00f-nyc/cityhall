...
class TestMemoryDbWithEnv(TestCase):
    ...
    def test_get_rights_on_non_user_fails(self):
        self.assertEqual(-1, self.db.get_rights('test', 'auto'))

    def test_get_rights_works(self):
        self.assertEqual(Rights.Grant, self.db.get_rights('auto', 'cityhall'))

    def test_create_user(self):
        count_before = len(self.db.authTable)
        self.db.create_user('cityhall', 'test', '')
        count_after = len(self.db.authTable)
        self.assertEqual(count_before + 1, count_after)

        last_entry = self.db.authTable[count_before]
    ...
    def test_set_rights_works(self):
        self.db.create_user('cityhall', 'test', '')
        original_auth = self.db.authTable[len(self.db.authTable)-1]

        self.db.update_rights('cityhall', 'auto', 'test', Rights.Read)
        new_auth = self.db.authTable[len(self.db.authTable)-1]
    ...
    def test_set_rights_to_itself_is_noop(self):
        self.db.create_user('cityhall', 'test', '')
        before = len(self.db.authTable)
        self.db.update_rights('cityhall', 'auto', 'test', Rights.NoRights)
        after = len(self.db.authTable)
        self.assertEqual(before, after)

    def test_create_rights_to_different_env(self):
        self.db.create_user('cityhall', 'test', '')
        self.db.create_env('cityhall', 'dev')
        before = len(self.db.authTable)
        self.db.create_rights('cityhall', 'dev', 'test', Rights.Read)
        after = len(self.db.authTable)
        self.assertEqual(before+1, after)

    def test_authenticate(self):
        self.assertFalse(self.db.authenticate('test', ''))
        self.db.create_user('cityhall', 'test', '')
        self.assertTrue(self.db.authenticate('test', ''))
        self.assertTrue(self.db.authenticate('test', '', 'auto'))
        self.assertFalse(self.db.authenticate('test', '', 'dev'))

    def test_create_env(self):
        vals_before = len(self.db.valsTable)
        auth_before = len(self.db.authTable)
        self.db.create_env('cityhall', 'dev')
        vals_after = len(self.db.valsTable)
        auth_after = len(self.db.authTable)
    ...
    def test_create_same_env_is_noop(self):
        ...
        auth_before = len(self.db.authTable)
        self.db.create_env('cityhall', 'dev')
        self.assertEqual(vals_before, len(self.db.valsTable))
        self.assertEqual(auth_before, len(self.db.authTable))

    def test_create_env_creates_root(self):
        self.db.create_env('cityhall', 'dev')

        val = self.db.valsTable[-1]
    ...
    def test_create_env_on_non_user_fails(self):
        ...
        self.assertEqual(before, len(self.db.valsTable))

    def test_get_env_root_on_nonexistant(self):
        self.assertEqual(-1, self.db.get_env_root('dev'))

    def test_get_env_root_on_existant(self):
        self.db.create_env('cityhall', 'dev')
        val = self.db.valsTable[-1]
        self.assertEqual(val['id'], self.db.get_env_root('dev'))
...
class TestMemoryDbWithEnvAndUser(TestCase):
    ...
    def test_update(self):
        ...
        original_value = self.db.valsTable[-1]
        before = len(self.db.valsTable)
        self.db.update('test', original_value['id'], 'another value')
        after = len(self.db.valsTable)
        new_value = self.db.valsTable[before]
    ...
    def test_get_value(self):
        dev_root = self.db.get_env_root('dev')
        self.db.create('test', 'dev', dev_root, 'value1', 'some value')
        item_in_db = self.db.valsTable[-1]
